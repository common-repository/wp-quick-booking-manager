...
function sc_scbooking_install() {
  global $table_prefix, $wpdb;
  $table_name1 = $table_prefix.'sc_scbooking';
  $sql_sfe = "CREATE TABLE IF NOT EXISTS  $table_name1(
   ...
   $wpdb->query($sql_sfe);
	 //-------------------------------
   $table_name2 = $table_prefix.'sc_scbooking_paymentmethods';
   ...
   $wpdb->query($sql_scpm);	 
    ...
    //$curr_count = $wpdb->query( "SELECT COUNT(*) FROM ".$table_currency."");   
	$curr_count = $wpdb->get_results( "SELECT COUNT(*) as rowcount FROM ".$table_currency);   
	
	if(!$curr_count[0]->rowcount){
		$currency_tbl_data = "INSERT INTO `".$table_currency."` (`id`, `country`, `isocode`, `currency`, `symbol`, `symbol_html`, `code`, `has_regions`, `tax`, `continent`, `visible`) VALUES (1, 'Mauritania', 'MR', 'Mauritanian Ouguiya', '', '', 'MRO', '0', '0', 'africa', '1'),
  ...
  if(empty($scbooking_calendar_page)){
    $scbooking_calendar_page_id = sc_programmatically_create_page('Booking Calendar','booking-calendar','[sc_sccalendar]','page');
  }
  if(empty($scrooms_page)){
    $sc_rooms_id = sc_programmatically_create_page('Rooms','room-gallery','[sc_roomsgallery]','page');
  }
  if(empty($roomdetails_page)){
    $room_details_id = sc_programmatically_create_page('Room Details','room-details','[sc_scbooking_roomdetails]','page');
  } 
}